...
assert.match(approval, /A webhook URL is simply the return address Make creates for the decision/);
assert.match(approval, /Set the window to 60 seconds/);
assert.match(approval, /Use this template in <span id="platformName">your automation<\/span>/);
assert.match(approval, /Advanced setup and structured data/);
assert.match(approval, /data-delivery="email"/);
assert.match(approval, /data-delivery="slack"/);
assert.match(approval, /data-delivery="link"/);
assert.match(approval, /Choose where they receive it\. The review stays the same\./);
assert.match(approval, /60 seconds → Approved → Make callback → Send once/);
assert.match(approval, /60 seconds → Reject → Make callback → Block/);
assert.match(approval, /SHSY handles the page, waiting, safeguards and result delivery/);
