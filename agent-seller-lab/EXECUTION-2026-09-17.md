...
## Validation record
19 focused seller tests pass using isolated mock settlement, not real payments. Existing basic/key review, recovery, restart, tenant isolation, key migration, load, notification and template suites pass. Existing `test:reliability` fails on old admin UI text (`Operations`) in both unchanged main and this branch; no production behavior was altered to appease that assertion. Dependency audit: zero reported vulnerabilities after compatible patch updates. Local product page was visually inspected.

## Deployment checkpoint
Initial implementation merged in PR #2, commit `8e83c6e`, deployed by the existing Railway GitHub integration. Public `/agent-seller/status` returned `accepting_payments:false` on 2026-09-17 KST, accurately reflecting the missing receiver. The x402scan discovery specification was reviewed directly: canonical `/openapi.json`, `info.x-guidance`, operation `x-payment-info`, input/output schema and an unauthenticated 402 probe are required. Follow-up adds these before submission. No listing submission has been made while payments are disabled.
